...
1. **dataent.ui.form.on(*doctype*, *fieldname*, *handler*)** is used to bind a handler to the event when the property library_member is set.
1. In the handler, we trigger an AJAX call to `dataent.client.get`. In response we get the requested object as JSON. [Learn more about the API](/dataent/user/en/guides/integration/rest_api).
1. Using **dataent.model.set_value(*doctype*, *name*, *fieldname*, *value*)** we set the value in the form.
