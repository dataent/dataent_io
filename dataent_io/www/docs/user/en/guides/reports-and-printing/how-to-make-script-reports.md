...
Since these reports give you unrestricted access via Python scripts, they can only be created by Administrators. The script part of the report becomes a part of the repository of the application. If you have not created an app, [read this](https://dataent.io/docs/user/en/guides/app-development/).

> Note: You must be in [Developer Mode](https://dataent.io/docs/user/en/guides/app-development/how-enable-developer-mode-in-dataent) to do this
...
1. In the module folder (for example if it is Accounts in EPAAS the folder will be `epaas/accounts/report/[report-name]`) you will see that templates for the report files will be created.
...
You can add filters in the `.js`. See an example below:
	...
	dataent.query_reports["Accounts Receivable"] = {
		"filters": [
			{
				"fieldname":"company",
				"label": __("Company"),
...
4. For example see existing reports. ([Balance Sheet](https://github.com/dataent/epaas/blob/develop/epaas/accounts/report/balance_sheet/balance_sheet.py))
