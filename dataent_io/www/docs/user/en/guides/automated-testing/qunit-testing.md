...
QUnit helps you write UI tests using the UQuit framework and native dataent API. As you might have guessed, this is a much faster way of writing tests.
...
In the CI, all QUnit tests are run by the **Test Runner** using `dataent/tests/test_test_runner.py`
...
To run a Test Runner based test, use the `run-ui-tests` bench command by passing the name of the file you want to run.
	...
	bench run-ui-tests --test dataent/tests/ui/test_list.js
...
Here is the example of the To Do test in QUnit
	...
	QUnit.test("Test quick entry", function(assert) {
		...
		let random_text = dataent.utils.get_random(10);

		dataent.run_serially([
			() => dataent.set_route('List', 'ToDo'),
			() => dataent.new_doc('ToDo'),
			() => dataent.quick_entry.dialog.set_value('description', random_text),
			() => dataent.quick_entry.insert(),
			(doc) => {
				assert.ok(doc && !doc.__islocal);
				return dataent.set_route('Form', 'ToDo', doc.name);
			},
			...
			() => dataent.tests.click_page_head_item('Yes'),
