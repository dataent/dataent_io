...
For example, [EPAAS](/apps/epaas)'s hooks contains a line,

	after_install = "epaas.setup.install.after_install"

So, the function after\_install is imported and called after EPAAS is installed.
...
`dataent.boot`. By default, the `bootinfo` contains
...
The notification configuration hook is expected to return a Python dictionary.
	...
	{ 
		...
		"for_module": {
			"To Do": "dataent.core.notifications.get_things_todo",
			"Calendar": "dataent.core.notifications.get_todays_events",
			"Messages": "dataent.core.notifications.get_unread_messages"
		}
	}
...
Note: to create an asset bundle (eg, assets/js/epaas.min.js) the target file
...
Example of such a function,
	...
	def get_permission_query_conditions():
		return "(tabevent.event_type='public' or tabevent.owner='{user}'".format(user=dataent.session.user)
...
For Example,
	...
	def has_permission(doc):
		if doc.event_type=="Public" or doc.owner==dataent.session.user:
			return True
...
Example,
	...
	scheduler_events = {
		"{daily}": [
			"epaas.accounts.doctype.sales_invoice.sales_invoice.manage_recurring_invoices"
		],
		"{daily_long}": [
			"epaas.setup.doctype.backup_manager.backup_manager.take_backups_daily"
		],
		"cron": {
			"15 18 * * *": [
