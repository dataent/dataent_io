...
You can setup the bench for production use by configuring two programs, Supervisor and nginx. If you want to revert your Production Setup to Development Setup refer to [these commands](https://github.com/dataent/bench/wiki/Stopping-Production-and-starting-Development)
...
```
bench setup supervisor
sudo ln -s `pwd`/config/supervisor.conf /etc/supervisor/conf.d/dataent-bench.ini #for CentOS 7 only
```
...
requests to dataent. You can generate the required configuration for nginx using
