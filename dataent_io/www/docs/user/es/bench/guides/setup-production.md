...
Puedes configurar el bench para producción configurando dos parametros, Supervisor y nginx. Si quieres volver a ponerlo en desarrollo debes ver [estos comandos](https://github.com/dataent/bench/wiki/Stopping-Production-and-starting-Development)
...
sudo ln -s `pwd`/config/supervisor.conf /etc/supervisor/conf.d/dataent-bench.conf
```

Nota: Para CentOS 7, la extensión debería ser `ini`, por lo que el comando sería
```
bench setup supervisor
sudo ln -s `pwd`/config/supervisor.conf /etc/supervisor/conf.d/dataent-bench.ini #para CentOS 7 solamente
```
...
peticiones a dataent. Puedes generar las configuraciones necesarias para nginx usando el comando `bench setup nginx`.
...
```
bench setup nginx
sudo ln -s `pwd`/config/nginx.conf /etc/nginx/conf.d/dataent-bench.conf
```
