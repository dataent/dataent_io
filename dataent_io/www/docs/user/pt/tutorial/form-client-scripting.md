...
1. **dataent.ui.form.on(*doctype*, *fieldname*, *handler*)** é utilizado para ligar um handler ao evento quando a propriedade library_member for definida.
1. No handler, nós desencadear uma chamada AJAX para `dataent.client.get`. Em resposta obtemos o objeto solicitado como JSON. [Saiba mais sobre a API](/dataent/user/en/guides/integration/rest_api).
1. Utilizando **dataent.model.set_value(*doctype*, *name*, *fieldname*, *value*)** Nós inserimos o valor no formulário.
