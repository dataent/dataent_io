...
1. Pegamos a última transação antes da data da transação atual usando a função de consulta `dataent.get_list`
1. Se a última transação for algo que não queremos, lançamos uma exceção usando `dataent.throw`
